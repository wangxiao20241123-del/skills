...
- 上游：`softaworks/agent-toolkit`
- 本仓库来源名：`softaworks`
- 上游原版：`upstream/softaworks/agent-toolkit/skills/...`
- 自定义发布版：`softaworks/<skill>/...`
...
upstream/softaworks/agent-toolkit/skills/... # softaworks/agent-toolkit 原版；用于对照和合并上游更新
softaworks/<skill>/...                     # 自定义发布版；从这里安装
```
...
- `upstream/softaworks/...` 不直接发布，只用于保存上游原版。
- `softaworks/...` 是发布版；当前 `mermaid-diagrams` 不做修改，必须保持和上游原版一致。
...
安装本仓库自定义版 `softaworks` 里的 `mermaid-diagrams`：

```bash
npx skills@latest add https://github.com/wangxiao20241123-del/skills/tree/main/softaworks \
  --agent codex \
...
```bash
git add upstream matt vercel-labs softaworks README.md NOTICE.md
scripts/validate-skill-layout.sh
git commit -m "add softaworks mermaid diagrams skill"
git push
```
